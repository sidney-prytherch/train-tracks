...
//    private lateinit var trainAnimation: AnimationDrawable
    ...
    private var animR: AnimatedVectorDrawableCompat? = null
    ...
    override fun onCreateView(
        ...
        animR = AnimatedVectorDrawableCompat.create(binding.trainImage.context, R.drawable.animated_train)
        animR?.let {
            binding.trainImage.setImageDrawable(it)
        }
        binding.button.setOnClickListener {
            (binding.trainImage.drawable as Animatable?)?.start()
            findNavController().navigate(R.id.action_nav_home_to_nav_puzzle)
        }
        return root
    }
